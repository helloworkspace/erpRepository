...
import com.yla.entity.SaleList;
import com.yla.entity.SaleListGoods;
import com.yla.repository.GoodsRepository;
import com.yla.repository.GoodsTypeRepository;
import com.yla.repository.SaleListGoodsRepository;
import com.yla.repository.SaleListRepository;
import com.yla.service.SaleListService;
		...
		return saleListRepository.findAll(new Specification<SaleList>(){
			...
			public Predicate toPredicate(Root<SaleList> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(saleList!=null){
					...
					if(saleList.getSalesman()!=null && saleList.getSalesman().getId()!=null){
						predicate.getExpressions().add(cb.equal(root.get("salesman").get("id"), saleList.getSalesman().getId()));
					}
