...
import com.yla.entity.CustomerReturnList;
import com.yla.entity.CustomerReturnListGoods;
import com.yla.repository.GoodsRepository;
import com.yla.repository.GoodsTypeRepository;
import com.yla.repository.CustomerReturnListGoodsRepository;
import com.yla.repository.CustomerReturnListRepository;
import com.yla.service.CustomerReturnListService;
		...
		return customerReturnListRepository.findAll(new Specification<CustomerReturnList>(){
			...
			public Predicate toPredicate(Root<CustomerReturnList> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(customerReturnList!=null){
					...
					if(customerReturnList.getSalesman()!=null && customerReturnList.getSalesman().getId()!=null){
						predicate.getExpressions().add(cb.equal(root.get("salesman").get("id"), customerReturnList.getSalesman().getId()));
					}
