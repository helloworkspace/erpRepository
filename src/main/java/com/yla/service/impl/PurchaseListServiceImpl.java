...
import com.yla.entity.PurchaseList;
import com.yla.entity.PurchaseListGoods;
import com.yla.repository.GoodsRepository;
import com.yla.repository.GoodsTypeRepository;
import com.yla.repository.PurchaseListGoodsRepository;
import com.yla.repository.PurchaseListRepository;
import com.yla.service.PurchaseListService;
import com.yla.util.MathUtil;
		...
		return purchaseListRepository.findAll(new Specification<PurchaseList>(){
			...
			public Predicate toPredicate(Root<PurchaseList> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(purchaseList!=null){
					...
					if(purchaseList.getBuyer()!=null && purchaseList.getBuyer().getId()!=null){
						predicate.getExpressions().add(cb.equal(root.get("buyer").get("id"), purchaseList.getBuyer().getId()));
					}
