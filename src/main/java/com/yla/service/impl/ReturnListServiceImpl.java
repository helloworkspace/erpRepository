...
import com.yla.entity.ReturnList;
import com.yla.entity.ReturnListGoods;
import com.yla.repository.GoodsRepository;
import com.yla.repository.GoodsTypeRepository;
import com.yla.repository.ReturnListGoodsRepository;
import com.yla.repository.ReturnListRepository;
import com.yla.service.ReturnListService;
		...
		return returnListRepository.findAll(new Specification<ReturnList>(){
			...
			public Predicate toPredicate(Root<ReturnList> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(returnList!=null){
					...
					if(returnList.getBuyer()!=null && returnList.getBuyer().getId()!=null){
						predicate.getExpressions().add(cb.equal(root.get("buyer").get("id"), returnList.getBuyer().getId()));
					}
